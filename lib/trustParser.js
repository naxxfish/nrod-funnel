...
function movements_movement(body, header)
{
	...
	trains.update({'trustID': body.train_id}, record, {upsert:true},  () => {
...
function movements_activation(body, header)
{
	...
	schedule.findOne({
		...
		trains.update({'descr': trainDescr} , record, {upsert: true},  (error, record) => {
...
function movements_cancellation (body, header)
{
	...
	trains.update({'trustID': body.train_id},record,  (error, record) => {
