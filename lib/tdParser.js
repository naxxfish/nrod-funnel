...
function processC_MSG(msgType, message)
{
	...
	{
		case "CA":
			berths.update({'berth': message.to}, {$push: {'describers': message.descr}}, {upsert:true},  (err, update) => {
				debug('Moved IN ' + message.descr + ' into berth ' + message.to)
			})
			berths.update({'berth': message.from}, {$pull: {'describers': message.descr}},  (err,update) => {
		...
		case "CB":
			...
			break;
		case "CC":
			berths.update({'berth': message.to}, {$set: {'describers': [ message.descr ]}}, {upsert:true},  (err,update) => {
				debug('Interposed ' + message.descr + ' into berth ' + message.to)
			})
	}
	smart.findOne({'FROMBERTH': message.from, 'TOBERTH': message.to, 'STEPTYPE': 'B'}, (err, berth)  => {
		if (berth != null)
		{
			corpus.findOne({'STANOX': berth.STANOX}, (err, stanox) => {
				if (stanox != null)
				{
					var reference = db.collection('REFERENCE')
					reference.findOne({'TIPLOC': stanox.TIPLOC, 'refType': 'GeographicData'}, (err, location) =>
					{
						var record = {
						...
						trains.update({'descr': message.descr} , {$set: record } , {upsert: true},  (error, myRecord) => {
